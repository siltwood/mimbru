...
export default function CreatureScreen() {
	...
	// Get urgency warnings based on thresholds
		...
		<ScrollView className="flex-1 bg-background">
			...
			<View className="space-y-3">
				...
				<Button 
					...
					disabled={creature.is_dead}
				>
					<Text className={creature.is_dead ? "text-gray-400" : ""}>
						{creature.is_dead ? "😵 Can't clean (Passed out)" : `🧼 Clean Pet ${creature.poop_count > 0 ? `(${creature.poop_count} 💩)` : ""}`}
					</Text>
				</Button>
				...
				>
					<Text className={creature.is_dead ? "text-gray-400" : ""}>
						{creature.is_dead ? "😵 Can't pet (Passed out)" : "😍 Pet Your Creature"}
					</Text>
				</Button>
			</View>
